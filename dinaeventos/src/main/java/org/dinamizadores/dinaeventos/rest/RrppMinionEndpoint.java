...
@Stateless
@Path("/rrppminions")
public class RrppMinionEndpoint {
	@PersistenceContext(unitName = "dinaeventos-persistence-unit")
	private EntityManager em;

	@POST
	@Consumes("application/json")
	public Response create(RrppMinion entity) {
		em.persist(entity);
		return Response
				.created(
						UriBuilder.fromResource(RrppMinionEndpoint.class)
								.path(String.valueOf(entity.getIdrrppMinion()))
								.build()).build();
	}
	...
	@GET
	@Produces("application/json")
	public List<RrppMinion> listAll(@QueryParam("start") Integer startPosition,
		...
		final List<RrppMinion> results = findAllQuery.getResultList();
		return results;
	}
	...
	@Path("/{id:[0-9][0-9]*}")
	@Consumes("application/json")
	public Response update(@PathParam("id") int id, RrppMinion entity) {
		if (entity == null) {
			return Response.status(Status.BAD_REQUEST).build();
		}
		if (id != entity.getIdrrppMinion()) {
			return Response.status(Status.CONFLICT).entity(entity).build();
		}
		if (em.find(RrppMinion.class, id) == null) {
			return Response.status(Status.NOT_FOUND).build();
		}
		try {
