...
public class EntradaDao {
	...
	public Entrada findByNumeroSerie(String numeroSerie) {
		TypedQuery<Entrada> findByNumeroSerie = em.createQuery("SELECT distinct e FROM Entrada e JOIN FETCH e.usuario JOIN FETCH e.ddTipoEntrada where e.numeroserie = :numeroSerie", Entrada.class);
	...
	public List<Entrada> findByField(String textoBusqueda) {
		TypedQuery<Entrada> findByField = em.createQuery("SELECT distinct e FROM Entrada e where e.usuario.dni LIKE :dni OR usuario.nombre LIKE :nombre" + " OR usuario.apellidos LIKE :apellidos OR usuario.email LIKE :email", Entrada.class);
	...
	public List<Entrada> listByEventID(int idEvento) {
		TypedQuery<Entrada> findByEventID = em.createQuery("SELECT distinct e FROM Entrada e JOIN FETCH e.usuario where e.evento.idevento = :idevento", Entrada.class);
	...
	public List<Entrada> listAll(Integer startPosition, Integer maxResult) {
		TypedQuery<Entrada> findAllQuery = em.createQuery("SELECT DISTINCT e FROM Entrada e ORDER BY e.identrada", Entrada.class);
	...
	public List<DdTipoEntrada> listTipoEntrada(Integer idEvento) {
		TypedQuery<DdTipoEntrada> findAllQuery = em.createQuery("SELECT DISTINCT te FROM DdTipoEntrada te, Evento e WHERE e.idevento = :idEvento AND te.activa = 1 ORDER BY te.idtipoentrada ", DdTipoEntrada.class);
	...
	public List<DdTipoComplemento> listTipoComplemento(Integer idEvento) {
		TypedQuery<DdTipoComplemento> findAllQuery = em.createQuery("SELECT DISTINCT te FROM DdTipoComplemento te, Evento e WHERE e.idevento = :idEvento ORDER BY te.idTipoComplemento ", DdTipoComplemento.class);
		findAllQuery.setParameter("idEvento", idEvento);
		return findAllQuery.getResultList();
	}

	public Integer getEntradaDniEvento(Integer idUsuario, Integer idEvento, String dni) {
		TypedQuery<Integer> findAllQuery = em.createQuery("SELECT e.identrada FROM Entrada e JOIN usuario u ON u.idusuario = e.idusuario WHERE e.idusuario = :idUsuario AND e.idevento = :idEvento and u.dni = :dni", Integer.class);
